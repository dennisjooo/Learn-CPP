...
// Reflexive Association (a class that has a relationship with other objects of its own type)
class Course
{
private:
    std::string m_name;
...
// Indirect Association (a class that has a relationship with another class that has a relationship with the first class)
class Car
{
private:
	std::string m_name;
...
int main()
{
    ...
    Course intro_cs{ "Introduction to Programming"};
    Course oop{ "Object-Oriented Programming", &intro_cs }; // oop requires intro_cs as a prerequisite

    
    // Indirect Association
    Driver jeremy{ "Jeremy", 62 }; // Jeremy drives the car with ID 62

    Car* jeremysCar{ CarLot::getCar(jeremy.getCarId()) }; // get Jeremy's Car from the car lot

    std::cout << jeremy.getName() << " is driving a " << jeremysCar->getName() << '\n';
