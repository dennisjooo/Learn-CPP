...
{  
   ...
   double width; // Define width as a double variable

   int a, b; // Define multiple variables of the same type
   ...
   */
   ...
   int xxxx {}; // Initialize x to 0 (value initialization)

   std::cout << xxxx << std::endl; // prints 0

   // Initialization of multiple variables
   int xa { 5 }, ya { 6 }; // Initialize x to 5 and y to 6
   int xb = 5, yb = 6; // Initialize x to 5 and y to 6 (copy initialization)
   int xc(5), yc(6); // Initialize x to 5 and y to 6 (direct initialization)
   int i {}, j {}; // Initialize i and j to 0 (value initialization)

   std::cout << xa << std::endl; // prints 5

   // Maybe Unused Variables
   double pi {3.14}; // The compiler will not warn about this unused variable, -Werror will bite you in the ass if not
